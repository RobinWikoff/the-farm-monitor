...
@pytest.mark.integration
def test_app_loads_without_error_in_ci_non_live_mode(monkeypatch):
    monkeypatch.setenv("CI", "true")
    monkeypatch.setenv("ENV", "dev")
    monkeypatch.setenv("DEV_ALLOW_LIVE_API", "false")
    monkeypatch.setenv("DEV_USE_SAMPLE_DATA", "true")
    monkeypatch.setenv("RUN_LIVE_INTEGRATION_TESTS", "false")
    ...
    def fail_if_called(*args, **kwargs):
        raise AssertionError("requests.get should not be called in ci non-live mode")
