...
def show_dashboard():
    ...
    # 2. DATA PREP FOR SHARED COLOR SCALE
    # We combine Target, Actual, and Forecast into one logical color mapping
    ...
    # 3. CONSOLIDATED SCALE
    # This ensures everything appears in one legend
    color_scale = alt.Scale(
        domain=['Actual', 'Forecast', 'Target'],
        range=['#00f2ff', '#ffffff', '#32CD32'] # Cyan, White, Lime Green
    )
    ...
    main_chart = alt.Chart(plot_df).mark_line(strokeWidth=4).encode(
        ...
        strokeDash=alt.condition(alt.datum.Status == 'Target', alt.value([8, 4]), 
                    alt.condition(alt.datum.Status == 'Forecast', alt.value([5, 5]), alt.value([0])))
    )
